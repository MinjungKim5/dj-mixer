...
- 자동 선택: 동기화(Sync) 시 또는 곡이 처음 추가될 때, 아무 곡도 선택되지 않은 상태(`-1`)라면 자동으로 첫 번째 곡(`0`)을 가리킴
- 스마트 재생: 곡이 선택되지 않은 상태에서 재생 클릭 시 자동으로 첫 번째 곡부터 로드 및 재생
- 이어재생 최적화: 이미 로드된 곡은 처음부터 다시 시작하지 않고 멈춘 지점에서 이어서 재생
...
- [x] 자동 곡 선택 및 재생 로직 개선 (Sync 시 첫 곡 자동 지정, 재생 시 첫 곡 자동 로드, 이어재생 최적화)
